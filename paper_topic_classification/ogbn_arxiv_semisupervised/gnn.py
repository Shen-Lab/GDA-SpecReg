...
def train(model, data, train_idx, valid_idx, idx_semi, optimizer):
    model.train()

    optimizer.zero_grad()
    out = model(data.x, data.adj_t)[torch.cat([train_idx, valid_idx[idx_semi]])]
    loss = F.nll_loss(out, data.y.squeeze(1)[torch.cat([train_idx, valid_idx[idx_semi]])])
    loss.backward()
    optimizer.step()
...
def main():
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, data, train_idx[idx_rand], valid_idx, idx_semi, optimizer)
