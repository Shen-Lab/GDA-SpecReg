...
class DomainData(InMemoryDataset):
    ...
    def __init__(self,
        ...
        # if True:
            num_node = self.data.x.shape[0]
            edge_index, edge_weight = tgeom.utils.get_laplacian(self.data.edge_index, num_nodes=num_node)
            ...
            torch.save(torch.tensor( pca.explained_variance_ ** 0.5, dtype=torch.float32 ), self.processed_paths[0] + 'eival.pt')
            torch.save(torch.tensor( pca.components_, dtype=torch.float32 ), self.processed_paths[0] + 'eivec.pt')

        self.eival = torch.load(self.processed_paths[0] + 'eival.pt').cuda()
        self.eivec = torch.load(self.processed_paths[0] + 'eivec.pt').cuda()
